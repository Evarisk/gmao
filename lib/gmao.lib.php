 ...
 */
function gmao_admin_prepare_head(): array
{
    ...
    $head[$h][0] = dol_buildpath('/saturne/admin/about.php', 1) . '?module_name=GMAO';
    $head[$h][1] = $conf->browser->layout != 'phone' ? '<i class="fab fa-readme pictofixedwidth"></i>' . $langs->trans('About') : '<i class="fab fa-readme"></i>';
    $head[$h][2] = 'about';
    $h++;

    complete_head_from_modules($conf, $langs, null, $head, $h, 'gmao@gmao');

    complete_head_from_modules($conf, $langs, null, $head, $h, 'gmao@gmao', 'remove');
